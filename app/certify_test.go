...
func TestBuildWiresDefaultLanesIntoCertification(t *testing.T) {
	...
	gated, ok := b.gateway.CertificationClientForTest("2.2").(*engine.GatedCertificationValidator)
	if !ok {
		t.Fatalf("2.2 certification client = %#v, want the gated default wired from the lane manager", b.gateway.CertificationClientForTest("2.2"))
	}
	...
	if v, ok := b.gateway.CertificationClientForTest("2.1").(*shnsdk.OperationValidator); !ok || v.BaseURL != "http://127.0.0.1:9/fhir21" {
		t.Fatalf("2.1 certification client = %#v, want the configured lane's own client", b.gateway.CertificationClientForTest("2.1"))
	}
}
