...
Vendored copies of three resources used by `verify.sh` to prove the validator
resolves the Da Vinci PAS/DTR/PDex profiles **offline** (the network is isolated
during the proof). The check keys only on profile *resolution* (the absence of
