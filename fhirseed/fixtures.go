...
// demoProviderPersonas is the baked provider-tenant demo persona set,
...
// can resolve conformant-lane members against the local provider SoR.
//
//go:embed fixtures/demo-provider-personas.json
var demoProviderPersonas []byte

// DemoProviderPersonasBundle returns a copy of the baked provider-tenant
// persona transaction Bundle (PUT entries — idempotent to re-POST).
func DemoProviderPersonasBundle() []byte {
	return append([]byte(nil), demoProviderPersonas...)
}

// conformantPersonas is the baked conformant-lane Patient roster (6 members:
// MBR-COVERED / MBR-NOTCOVERED / MBR-UC06 / MBR-UC07HCPCS / MBR-UC08 /
// MBR-BRIDGE-DEMO), generated from the private canonical seed source and
// drift-guarded there (synthetic personas only, FR-39). Partners load it so
// the conformant-lane ingress member-fence resolves these members against
// their SoR.
//
...
// (no error) like DemoProviderPersonasBundle — it copies embedded bytes and
// cannot fail.
