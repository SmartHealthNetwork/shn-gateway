...
// uses (gateway/engine/originate.go) — no new shnsdk symbol (design §1.3).
package engine
...
func (n *nativeResponder) handlePASClaim(ctx context.Context, corrID, subjectPCI string, requestFHIR []byte) (LegResult, error) {
	...
	// A real Da Vinci $submit always returns a Bundle; SHN's canonical shape is either
	// a bare ClaimResponse (approved/denied) or a Bundle+Task (pended). Without
	// normalization, ParsePendedResponse would misread any top-level Bundle as pended.
