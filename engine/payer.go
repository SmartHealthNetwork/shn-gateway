...
// handleDTRInbound delivers the verified framed DTR operation through the
// shared native boundary; local-disclosure commands use separate handlers.
func (g *Gateway) handleDTRInbound(w http.ResponseWriter, r *http.Request, env shnsdk.Envelope, tok shnsdk.Token, reqJSON []byte, answerTok string) {
	g.handleNativeInbound(w, r, "dtr-questionnaire-fetch", env, tok, reqJSON, answerTok)
}
