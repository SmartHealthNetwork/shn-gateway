...
func TestHandleUC04_ProviderDataAttestsAndLeanTail(t *testing.T) {
	...
	// built-order `srRef` literal, in the provider-data lane.
	pdBranch := fn[gateIdx:]
	...
	if strings.Contains(pdBranch, "StoreAuthNumber(srRef") {
		t.Fatalf("provider-data lane must persist against the seeded order ref, not the built-order srRef literal")
	}
