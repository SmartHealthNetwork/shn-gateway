...
func (g *Gateway) inquiryLedgerEffect(requester, subject, boundPatientRef, legCorrID string, facts pasInquiryFacts, answer []byte, result *LegResult) (func() error, []ObserverEvent) {
	...
	for _, ans := range readPASInquiryAnswers(requester, answer) {
		...
		if eobErr != nil {
			...
			events = append(events, ObserverEvent{Kind: pendDecisionNotRecordedEvent, Direction: "ingress",
				LegType: "pas-claim-inquire", CorrelationID: legCorrID, Op: "pas-inquire",
	...
	commit := func() error {
		for _, d := range decisions {
			...
			if errors.Is(err, ErrEOBSubjectMismatch) {
				...
				// a decision EOB that cannot be built: the answer still reaches
				// the requester, this decision is not recorded, the operator is
				// told, and the answer's other decisions are recorded as usual.
				g.reportEOBOwnedElsewhere("pas-claim-inquire", "pas-inquire", legCorrID, d.corrID)
				continue
			}
			if err != nil {
				return err
...
func (g *Gateway) inquiryDecisionEOB(subjectPCI, corrID, boundPatientRef string, facts pasInquiryFacts, ans inquiryAnswerResponse, parsed shnsdk.PriorAuthResult) (*EOBRecord, error) {
	...
	return &EOBRecord{SubjectPCI: subjectPCI, EOBID: decisionEOBID(corrID), JSON: eobJSON}, nil
}
