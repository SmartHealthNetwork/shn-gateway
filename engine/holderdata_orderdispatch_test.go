...
// crd-order-select or PAS-tail legs instead — unaffected by this fixture set.
...
// pin: the in-process memstub payer (StubHolderData/stubPersonas — what every gateway
// process boots on when FHIR_DATA_URL is unset, incl. test/harness.go's payer and the
// SHN Kit's counterparty payer in test/kitlive) must ResolvePatient every provider-data
// order-dispatch persona's member id, exactly like the hosted payer already does
// (internal/fhirseed's payer-tenant Coverage carries MBR-OX/MBR-PD-UC03 — see
// fhirseed.go's demographics + seedCoverage tables). Without this, the payer-side
// crd-order-dispatch inbound bind (conformantCRDDispatchBind, crd_dispatch_native.go)
// 400s "unknown member" even though the provider side (reading off a real or
// bring-your-own FHIR SoR) resolves the member fine — this pin closes that census drift.
//
...
func TestStubPersonas_CoverOrderDispatchSeeds(t *testing.T) {
	...
	for _, name := range orderDispatchFixtures {
		...
		if _, _, found := stub.ResolvePatient(member); !found {
			t.Errorf("stubPersonas has no entry for %q (provider-data persona %q): the in-process payer would 400 %q on the crd-order-dispatch leg",
