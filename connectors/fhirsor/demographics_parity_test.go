...
// Source demographics retain the same reader semantics as a carried Patient,
// while network identity is the source's issued identifier, never their hash.
func TestResolvePatient_SourceIdentityAndDemographics(t *testing.T) {
	for name, patient := range map[string]string{
		"plain":      `{"resourceType":"Patient","id":"p1","name":[{"family":"Nakamura","given":["Iris"]}],"birthDate":"1962-03-11"}`,
		...
		t.Run(name, func(t *testing.T) {
			patient = strings.Replace(patient, `"id":"p1"`, `"id":"p1","identifier":[{"system":"`+shnsdk.MemberSystem+`","value":"m1"},{"system":"urn:shn:pci","value":"pci:issued"}]`, 1)
			...
			if pci != "pci:issued" {
				t.Fatalf("source-issued PCI replaced: %q", pci)
			}
		})
	}
